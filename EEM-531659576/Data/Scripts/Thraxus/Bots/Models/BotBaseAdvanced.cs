...
namespace Eem.Thraxus.Bots.Models
{
	public class BotBaseAdvanced
	{
		...
		private void SetFactionOwnership()
		{
			try
			{
				Marshall.WriteToLog("SetFactionOwnership", $"Setting faction ownership of {ThisEntity.DisplayName} to {MyAPIGateway.Session.Factions.TryGetFactionByTag(MyConfig.Faction).Tag}", true);
		...
		{   // Trigger alert, war, all the fun stuff against the player / faction that removed the block; also scan for main RC removal and shut down bot if Single Part
			...
			{
				Marshall.WriteToLog("OnBlockRemoved", $"Triggering shutdown for {ThisEntity.DisplayName} with ID {ThisEntity.EntityId}", true);
				BotShutdown?.Invoke();
			}
		}
		...
		{   // Trigger alert, war, all the fun stuff against the entity owner that triggered the integrity change (probably negative only)
			Marshall.WriteToLog("OnBlockIntegrityChanged", $"Block integrity changed for {ThisEntity.DisplayName} with ID {ThisEntity.EntityId} for block {block}", true);
			if (block.FatBlock == MyShipController && !MyShipController.IsFunctional)
			{
				Marshall.WriteToLog("OnBlockIntegrityChanged", $"Rc integrity compromised... triggering sleep for {ThisEntity.DisplayName} with ID {ThisEntity.EntityId}", true);
				Sleep();
				BotSleep?.Invoke();
			}
			...
			{
				Marshall.WriteToLog("OnBlockIntegrityChanged", $"Rc integrity restored... triggering wakeup for {ThisEntity.DisplayName} with ID {ThisEntity.EntityId}", true);
				Wakeup();
				BotWakeup?.Invoke();
			}
		}
		...
		{	// TODO make sure damage handlers are here since the bot is simulating offline here
			Marshall.WriteToLog("Sleep", $"Sleeping {ThisEntity.DisplayName} with ID {ThisEntity.EntityId}", true);
			_sleeping = true;
			ThisCubeGrid.OnBlockAdded -= OnBlockAdded;
		}
		...
		{	// TODO make sure damage handlers are here so we can properly wake the bot up
			_sleeping = false;
			Marshall.WriteToLog("Wakeup", $"Waking up {ThisEntity.DisplayName} with ID {ThisEntity.EntityId}", true);
			ThisCubeGrid.OnBlockAdded += OnBlockAdded;
		}
		...
		{   // Protection for initial spawn with MES, should be disabled after the first few seconds in game (~300 ticks)
			Marshall.WriteToLog("OnOnBlockOwnershipChanged", $"Ownership changed for {cubeGrid.DisplayName} with ID {cubeGrid.EntityId}", true);
